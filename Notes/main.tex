...
\item \textbf{Unknown Key-Share (UKS) attacks} -  This class of attacks occurs when two honest agent might derive the same key, but in effect did not expect to be talking together, this typically may occur with a client C willing to talk to a compromised server A, and the attacker makes it so that another server S derives the same key as C.
...
\begin{itemize}
\item \emph{non-injective unilateral authentication} - if the reader accepts some message as coming from some tag, the corresponding tag did send this message at some point in the past.
...
\paragraph{Properties} This protocol should provide two guarantees against an active attacker:
\begin{itemize}
\item \emph{injective unilateral client-side authentication} - if a client derives a key with some parameters (the set of public keys), a corresponding server session derived the same key with the same parameters.
...
\paragraph{Bonus}
\begin{enumerate}
\item Under compromise of long-term keys, i.e., attacker controlled servers, this protocol suffers from an Unknown Key-Share (UKS) attack. The corresponding attack trace is illustrated in \cref{app:attacks}, \cref{fig:kem-uks}.
...
\paragraph{Properties} This protocol should provide two guarantees against an active machine-in-the-middle attacker:
\begin{itemize}
\item \emph{injective authentication} - if a client derives a key with some parameters (the set of public keys), a corresponding server session derived the same key with the same parameters.
