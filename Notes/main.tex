...
% CC: Need to at some point add all authors that we missed here!
...
\begin{figure}[h]
 ...
 % \setlength{\belowcaptionskip}{-15pt}
    ...
    \begin{msc}{}
        ...
        \declinst{initiator}{$o_\sfsk, le_\sfpk$}{\textsc{Owner}}%
        \declinst{le}{$le_\sfsk$}{\textsc{Let's Encrypt (LE)}}%
        ...
        \action*{$
          \begin{array}{@{}l@{}}
          Generate \ token \\
          sig_{le} = \sign(token, le_\sfsk)   
          \end{array}$}{le}
        \nextlevel[5]
        \mess{$token, sig_{le}$}{le}{initiator}   
        \nextlevel[0.5]   
        \action*{$
          \begin{array}{@{}l@{}}
          \texttt{Abort if not }\verify(sig_{le}, token, le_\sfpk) \\
           sig = \sign(token, o_\sfsk)
          \end{array}$}{initiator}
        \nextlevel[3.5]                  
        \mess[dotted]{DNS-Upd(Adr, $sig$)}{initiator}{receiver}
        \nextlevel[1.5]
        \mess{ Update $Adr, o_\sfpk$}{initiator}{le}
        \nextlevel[0.5]
...
\begin{figure}[h]
 ...
 % \setlength{\belowcaptionskip}{-15pt}
    ...
    \begin{msc}{}
        ...
        \declinst{initiator}{$le_\sfpk$}{\textsc{\textcolor{red}{Attacker}}}%
        \declinst{le}{$le_\sfsk$}{\textsc{Let's Encrypt (LE)}}%
        \declinst{receiver}{$Adr$}{\textsc{DNS Server}}%
         \nextlevel[1]
        \mess{Register, $Adr$}{initiator}{le}
        ...
        \action*[red]{$
          ...
          \end{array}$}{initiator}
        \nextlevel[3]                  
        \mess{\textcolor{red}{Update $Adr, pk_{deo}$}}{initiator}{le}
        \nextlevel[0.5]
        \mess{DNS-Req($Adr$)}{le}{receiver}
        \nextlevel[1.5]        
        \mess[dotted]{DNS-Rsp($sig$)}{receiver}{le}
